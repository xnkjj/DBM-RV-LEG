...
local function IsTransportGossip()
	local gossipFrame = _G["GossipFrame"]
    local specificName = UnitName("player")
	if gossipFrame and gossipFrame:IsShown() then
        local gossipTitle = GossipFrameNpcNameText:GetText()
		if gossipTitle == specificName then
			return true
		end
	end
	return false
...
function Mod:GOSSIP_SHOW()
	...
	if Addon.Config.autoGossip and scenarioType == LE_SCENARIO_TYPE_CHALLENGE_MODE and not npcBlacklist[npcId] and not IsTransportGossip() then
