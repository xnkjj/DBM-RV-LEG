...
Options.HeaderText:SetText(L.addonname)
...
local OptionsFrame = ExRT.Options:Add(L.addonname)
...
ExRT.Options.InBlizzardInterface.name = L.optionframename
...
ExRT.Options.InBlizzardInterface.button = ELib:Button(ExRT.Options.InBlizzardInterface,L.openoptionframe,0):Size(400,25):Point("TOP",0,-100):OnClick(function ()
	if InterfaceOptionsFrame:IsShown() then
		InterfaceOptionsFrame:Hide()
	end
	ExRT.Options:Open()
...
OptionsFrame.title = ELib:Text(OptionsFrame,L.addonname,22):Size(500,22):Point("LEFT",OptionsFrame.image,"RIGHT",20,0):Color()
