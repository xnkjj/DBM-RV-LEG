...
local sndYB		= mod:SoundMM("SoundYB")
	...
	elseif args:IsSpellID(124862) then
		...
		if self:AntiSpam(5, 3) then
			...
			if MyTT() then
				...
				sndWOP:Schedule(1, "ex_mop_zlzb") --戰慄準備
				sndWOP:Schedule(1.5, "countthree")
				sndWOP:Schedule(2.5, "counttwo")
				sndWOP:Schedule(3.5, "countone")
			end
...
function mod:UNIT_SPELLCAST_SUCCEEDED(uId, _, _, _, spellId)
	if spellId == 125098 then--Yell is about 1.5 seconds faster then this event, BUT, it also requires localizing. I don't think doing it this way hurts anything.
		...
		sndWOP:Schedule(152, "ex_mop_nwzb") --女王準備
		sndWOP:Schedule(153, "countfour")
		sndWOP:Schedule(154, "countthree")
		sndWOP:Schedule(155, "counttwo")
		sndWOP:Schedule(156, "countone")
		if self.Options.InfoFrame then
	...
	elseif spellId == 125304 then
		...
		ptwo = false
		sndWOP:Play("phasechange")--階段轉換
		sndWOP:Schedule(147, "ptwo")--P2
		sndWOP:Schedule(148, "countfour")
		sndWOP:Schedule(149, "countthree")
		sndWOP:Schedule(150, "counttwo")
		sndWOP:Schedule(151, "countone")
